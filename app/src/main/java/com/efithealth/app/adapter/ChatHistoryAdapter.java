...
public class ChatHistoryAdapter extends ArrayAdapter<EMContact> {
	...
	public ChatHistoryAdapter(Context context, int textViewResourceId,
			List<EMContact> objects, List<GroupList> list) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (list!=null) {
			for (int i = 0; i < list.size(); i++) {
				if (username.equals(list.get(i).getImid())) {
					ImageLoader.getInstance().displayImage(
							list.get(i).getImgsfilename(), holder.avatar);
					holder.name.setText(list.get(i).getGname());
				}
			}
		...
		if (conversation.getMsgCount() != 0) {
			...
			holder.message
					.setText(SmileUtils.getSmiledText(
							getContext(),
							CommonUtils.getMessageDigest(lastMessage,
